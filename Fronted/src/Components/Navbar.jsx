...
const handleSearchuser=async(e)=>{
  ...
  const renderMenu = (
    ...
    >
      <MenuItem onClick={handleMenuClose}>Social Page</MenuItem>
      ...
      <MenuItem > <p onClick={()=>dispatch(userLogOut())}>Logout</p></MenuItem>
    </Menu>
  );
      ...
      <AppBar position="fixed">
        <Toolbar>
          ...
          <Search >
            ...
            <StyledInputBase onChange={handleSearchuser}  
              placeholder="Search…"
              ...
              <div className='bg-white text-black absolute top-full w-full'>
          {
            SearchUser.map((ele,i)=>{
              return ele._id===userSlice.user?._id? <Link onClick={()=>setSearchUser([])} state={ele._id} to={'/profile'}  className='flex items-center gap-4 px-2 py-2 border-b border-[#e4dfdf]'>
              <img className='w-12 h-12 rounded-full border-amber-500 border' src={ele.profilePic} alt="" />
              <h3>{ele.name}</h3>
              </Link>
              :
              <Link onClick={()=>setSearchUser([])} state={ele._id} to={`/friendProfile?name=${ele.name} && id=${ele._id}`} className='flex items-center gap-4 px-2 py-2 border-b border-[#e4dfdf]'>
              <img className='w-12 h-12 rounded-full border-amber-500 border' src={ele.profilePic} alt="" />
              <h3>{ele.name}</h3>
              </Link>
            })
